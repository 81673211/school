...
$(function(){
    ...
    jQuery.validator.addMethod("maxValue", function(value, element, param) {   
         return this.optional(element) || parseFloat(value.split(",").join("")) <= param;       
    }, "不能大于设置值"); 
    // 判断小数点后的位数
    jQuery.validator.addMethod("isFloatLength", function(value, element,params) {
    	 var a = value.indexOf(".")+1;  
    	 if(a == 0){ a = value.length;}  
    	 var b = value.length;  
    	 var c = b-a;  
    	 return this.optional(element) || c<=params; 
    }, "小数不能大于4位数"); 
});
//身份证号码的验证规则
