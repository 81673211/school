...
(function(a) {
	a.fn.bsgrid_paging = {
		...
		createPagingToolbar : function(b) {
			...
			e
					.append('<table class="bsgridPaging' + (c ? " pagingLittleToolbar" : "") + (b.settings.pageSizeSelect ? "" : " noPageSizeSelect")
							+ '">');
			e.append("<tr>");
			...
			e.append("<td>");
			e.append('<input class="' + d + ' firstPage" type="button" id="' + b.firstPageId + '" value="'
					+ (c ? "" : a.bsgridLanguage.pagingToolbar.firstPage) + '" />');
			e.append("&nbsp;");
			e.append('<input class="' + d + ' prevPage" type="button" id="' + b.prevPageId + '" value="'
			...
			e.append("<td>");
			e.append('<input class="' + d + ' nextPage" type="button" id="' + b.nextPageId + '" value="'
					+ (c ? "" : a.bsgridLanguage.pagingToolbar.nextPage) + '" />');
			e.append("&nbsp;");
			e.append('<input class="' + d + ' lastPage" type="button" id="' + b.lastPageId + '" value="'
			...
			e.append('<input class="' + d + ' gotoPage" type="button" id="' + b.gotoPageId + '" value="'
					+ (c ? "" : a.bsgridLanguage.pagingToolbar.gotoPage) + '" />');
			e.append("</td>");
			e.append('<td class="refreshPageTd">');
			e.append('<input class="' + d + ' refreshPage" type="button" id="' + b.refreshPageId + '" value="'
	...
	$.fn.bsgrid = {
		...
		init : function(gridId, settings) {
			...
			if ($("#" + gridId).find("thead").length == 0) {
				...
				$("#" + gridId).find("tr:lt(" + ($("#" + gridId + " tr").length - $("#" + gridId + " tfoot tr").length) + ")").appendTo(
						$("#" + gridId + " thead"))
			}
			...
			var gridObj = {
				...
				search : function(params,curPage) {
					if(curPage>0){
		...
		parseJsonData : {
			success : function(json) {
				return json.success||json.code=="0";
			},
		...
		buildXmlData : {
			gridData : function(curPage, data) {
				return '<?xml version="1.0" encoding="UTF-8"?><gridData><success>true</success><totalRows>'
						+ $("<xml>" + data + "</xml>").find("row").length + "</totalRows><curPage>" + curPage + "</curPage><data>" + data
						+ "</data></gridData>"
			}
		},
		parseXmlData : {
		...
		getPageCondition : function(curPage, options) {
			...
			condition += (condition.length == 0 ? "" : "&") + options.settings.requestParamsName.pageSize + "=" + options.settings.pageSize + "&"
					+ options.settings.requestParamsName.curPage + "=" + curPage + "&" + options.settings.requestParamsName.sortName + "="
					+ options.sortName + "&" + options.settings.requestParamsName.sortOrder + "=" + options.sortOrder;
			return condition
		},
		search : function(params, options) {
			options.otherParames = params;
			$.fn.bsgrid.page(options.curPage, options)
		},
		page : function(curPage, options) {
			...
			if (options.settings.localData != false) {
				if (dataType == "json") {
					...
					if (dataType == "xml") {
						$.fn.bsgrid.loadGridData(dataType, "<xml>" + $.fn.bsgrid.buildData.gridData(dataType, curPage, options.settings.localData)
								+ "</xml>", options)
					}
				}
				return
			...
			$.ajax({
				...
				error : function(XMLHttpRequest, textStatus, errorThrown) {
					if($.isFunction(options.settings.error)){
						options.settings.error(options,XMLHttpRequest, textStatus, errorThrown);
					}else{
							...
							function(i) {
								...
								for ( var key in options.settings.event.customRowEvents) {
									trObj.bind(key, {
										...
										options.settings.event.customRowEvents[key](event.data.record, event.data.rowIndex, event.data.trObj,
												event.data.options)
									})
								}
								...
								$(this).find("td").each(
										function(j) {
											...
											for ( var key in options.settings.extend.renderPerColumnMethods) {
												var renderPerColumn_html = options.settings.extend.renderPerColumnMethods[key](record, i, j, tdObj,
														trObj, options);
											...
											for ( var key in options.settings.event.customCellEvents) {
												tdObj.bind(key, {
													...
													options.settings.event.customCellEvents[key](event.data.record, event.data.rowIndex,
															event.data.colIndex, event.data.tdObj, event.data.trObj, event.data.options)
												})
											}
										})
							})
		...
		createPagingOutTab : function(options) {
			...
			pagingOutTabSb.append('<table id="' + options.pagingOutTabId + '" class="bsgridPagingOutTab" style="display: none;"><tr><td align="'
		...
		addLockScreen : function(options) {
			if ($(".bsgrid.lockscreen").length == 0) {
				...
				lockScreenHtml.append('<table><tr><td><center><div class="bsgrid loading"><span>&nbsp;&emsp;</span>&nbsp;'
						+ $.bsgridLanguage.loadingDataMessage + "&emsp;<center></div></td></tr></table>");
				lockScreenHtml.append("</div>");
...
(function($) {
						...
						$(this).find("input[type=checkbox]").change(
								function() {
									...
									$.bsgrid.adaptAttrOrProp(
											$.fn.bsgrid.getRows(options).find("td:nth-child(" + (hi + 1) + ")>input[type=checkbox]"), "checked",
											checked)
								})
					}
				});
		...
		if (rowIndex < options.curPageRowsNum) {
			...
			if (columnModel.check == "true") {
				return '<input class="bsgrid_editgrid_check" type="checkbox" value="'
						+ $.fn.bsgrid.getRecordIndexValue(record, columnModel.index, options) + '"/>'
			}
		}
		return false
	};
				...
				tdObj.find(":input").each(
						function() {
							...
							formObj.bind(key, {
								...
								options.settings.event.customCellEditEvents[key](event.data.formObj, event.data.record, event.data.rowIndex,
										event.data.colIndex, event.data.tdObj, event.data.trObj, event.data.options)
							})
						})
			}
	...
	$.fn.bsgrid.extendAfterRenderGrid.aggregation = function(parseSuccess, gridData, options) {
		var gridObj = $.fn.bsgrid.getGridObj(options.gridId);
		var columnsModel = options.columnsModel;
		$("#" + options.gridId + " tfoot tr td[" + options.settings.colsProperties.aggAttr + "!='']")
				.each(
						function(i) {
							if (columnsModel[i].aggName != "") {
								...
								if (aggName == "count") {
									...
									if (aggName == "countnotnone" || aggName == "sum" || aggName == "avg" || aggName == "max" || aggName == "min"
											|| aggName == "concat") {
																...
																if (aggName == "countnotnone") {
																	...
																	if (aggName == "sum" || aggName == "avg") {
																		...
																		if (aggName == "max" || aggName == "min") {
																			if (!isNaN(rval)
																					&& (val == null || (aggName == "max" && parseFloat(rval) > val) || (aggName == "min" && parseFloat(rval) < val))) {
																				val = parseFloat(rval)
																			}
																		} else {
																			if (aggName == "concat") {
	...
	$.fn.bsgrid.defaults.extend.getRowsChangedColumnsValue = function(options) {
		...
		$
				.each(
						$.fn.bsgrid.defaults.extend.getChangedRowsIndexs(options),
						function(i, rowIndex) {
											...
											function(ci) {
												if ($(this).find(".bsgrid_editgrid_change").length > 0) {
													values["row_" + rowIndex][options.columnsModel[ci].index] = options.settings.extend.settings.gridEditConfigs[options.columnsModel[ci].edit]
															.val($(this).find(".bsgrid_editgrid_change"))
												}
											})
						});
		return values
	...
	$.fn.bsgrid.extendOtherMethods.fixedHeader = function(iFirst, options) {
		...
		$("#" + options.gridId + " thead tr:gt(" + (headTrNum - 1) + ")").each(
				function(ri) {
					$(this).find("th").each(
							function(i) {
								var thObj = $(this);
								$("#" + options.gridId + " thead tr:eq(" + ri + ") th:eq(" + i + ")").height(
										thObj.height() + ((ri == headTrNum - 1) ? 2 : 1) * getSize(thObj.css("border-top-width"))).width(
										thObj.width() + getSize(thObj.css("border-left-width")))
							})
				});
	...
	a.bsgrid_export = {
		...
		doExport : function(h, o, d) {
			...
			document.location.href = c.url + (c.url.indexOf("?") < 0 ? "?" : "&") + c.requestParamsName.exportFileName + "="
					+ encodeURIComponent(encodeURIComponent(c.exportFileName)) + "&" + c.requestParamsName.colNames + "="
					+ encodeURIComponent(encodeURIComponent(n.substring(1))) + "&" + c.requestParamsName.colIndexs + "=" + g.substring(1) + "&"
					+ c.requestParamsName.colWidths + "=" + m.substring(1) + "&" + c.requestParamsName.colAligns + "=" + j.substring(1)
					+ (b.length == 0 ? "" : ("&" + b))
		}
	}
})(jQuery);
